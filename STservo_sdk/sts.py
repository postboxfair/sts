...
class sts(protocol_packet_handler):
    ...
    # general write for single servo
    ...
    # Returns current speed per servo: sts_id
    ...
    # Returns current position per servo: sts_id
    ...
    # write synchronously for multi servos
    def SyncWritePosEx(self, sts_id, position, speed, acc):
        txpacket = [acc, self.sts_lobyte(position), self.sts_hibyte(position), 0, 0, self.sts_lobyte(speed), self.sts_hibyte(speed)]
        return self.groupSyncWrite.addParam(sts_id, txpacket)

    # position write asynchronously for single servo (call RegWriteAction to action)
    ...
    # eprom locked. Forbids manual rotation by hand and holds programmed position.
    def LockEprom(self, sts_id):
        return self.write1ByteTxRx(sts_id, STS_LOCK, 1)

    # eprom locked. Allows manual rotation by hand and does not hold programmed position.
